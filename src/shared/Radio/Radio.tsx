...
const Radio: FC<RadioProps> = ({ className, name, id, onChange, label, defaultChecked }) => {
  return (
    <div className={`flex items-center text-sm sm:text-base ${className}`}>
      <input
        ...
        value={id}
      />
      <label htmlFor={id} className="ml-2.5 sm:ml-3 block text-neutral-900 dark:text-neutral-100">
        {label}
      </label>
    </div>
  );
