...
export const mediaRunningSlice = createSlice({
  ...
  initialState,
  reducers: {
    changeCurrentMediaRunning: (state, action: PayloadAction<MediaRunningState>) => {
      return {
        ...state,
        ...action.payload
      };
    },
    changeStateMediaRunning: (state, action: PayloadAction<MediaRunningState['state']>) => {
      return {
        ...state,
...
export const selectCurrentMediaRunning = (state: RootState) => state.mediaRunning;
